    ...
    (function(THREE, $, BufferGeometry, Random, Band, Scene, Ellipsoid, Tranguloid, Pillow, OBJ_Tool, Robot) {
        ...
        var HtmlController = function(scene) {
            ...
            $('#btnEllipsoid').click(function() {
                ...
                if (umax_value > 2 * Math.PI) {
                    umax_value = 2 * Math.PI;
                };

                var vmax_value = parseFloat($('#numVmax').attr('value'));
                if (vmax_value > Math.PI) {
                    vmax_value = Math.PI;
                };

                var umin_value = parseFloat($('#numUmin').attr('value'));
                if (umin_value < 0) {
                    umin_value = 0;
                };

                var vmin_value = parseFloat($('#numVmin').attr('value'));
                if (vmin_value < 0) {
                    vmin_value = 0;
                };
            ...
            $('#btnTranguloid').click(function() {
                ...
                var umin_value = parseFloat($('#numUmin').attr('value'));
                if (umin_value < -Math.PI) {
                    umin_value = -Math.PI;
                };

                var vmin_value = parseFloat($('#numVmin').attr('value'));
                if (vmin_value < -Math.PI) {
                    vmin_value = -Math.PI;
                };
                var config = {
                    umin: umin_value,
